...
#ifndef MEMORYMAP_STOPWATCH_HPP
#define MEMORYMAP_STOPWATCH_HPP
...
namespace stopwatch
{
    ...
    private:
        ...
        template<TimeFormat fmt = TimeFormat::MILLISECONDS>
        static std::uint64_t
        ticks (const time_pt& start_time, const time_pt& end_time)
        {
            const auto duration = end_time - start_time;
            const std::uint64_t ns_count = std::chrono::duration_cast<std::chrono::nanoseconds>(
                duration).count();
            ...
            switch (fmt)
            {
                ...
                {
                    std::uint64_t up = ((ns_count / 100000000) % 10 >= 5) ? 1
                                                                          : 0;
    ...
    template <Stopwatch::TimeFormat format = Stopwatch::TimeFormat::MILLISECONDS>
    class ScopedStopwatch : Stopwatch
    {
        std::string name;
    public:
        ...
        ~ScopedStopwatch ()
        {
            std::cout << "Stopwatch " + name + " finished with time: "
                      << Stopwatch::elapsed<format>() << '\n';
        }
    };
